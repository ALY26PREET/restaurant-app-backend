...
class Product(Resource):
    def post(self):
        ...
        if 'name' in json and 'price' and 'category' in json:
            return mr.Product(json['category']).add(json['name'], json['price'])
    ...
    def get(self):
        ...
        json = json if json != None else {}
        if 'category' in json:
            return list(mr.Product(json['category']).getAll())
...
class Category(Resource):
    def post(self):
        ...
        json = json if json != None else {}
        if 'name' not in json:
            return {'status': 'Failed', message: 'Please include the \'name\' in the request'}
        else:
            res = mr.Category(restaurant).add(json['name'])
            return res

    def get(self):
        return list(mr.Category(restaurant).getAll())
